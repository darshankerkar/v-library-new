...
function Reserves() {
  ...
  <div className="logo pr-4 md:pr-8 py-2 shrink-0">
        ...
        <div className="hidden md:flex flex-1 items-center gap-x-8">
          ...
          <a href="/reserves" className="text-white text-xl transition duration-150"><u>Reserves</u></a> {/* Highlighted current page */}
        </div>
            ...
            {contacts.map((person, idx) => (
              <div key={idx} className="flex items-start mb-6 border-b pb-4 last:border-b-0 last:pb-0">
                <img
                  src={person.image}
                  alt={person.name}
                  className="w-16 h-16 rounded-full shadow-md object-cover mr-4 shrink-0"
